<?php
  ...
  class Route {
    ...
    function __construct() {
      $this->validTemplates = [ 'login', 'dashboard', 'install', 'settings', 'payload', 'reports', 'archive', 'report' ];
    ...
    * @return string          HTML for page
    ...
    public function template($file) {
      ...
      if($file == 'report' && ((is_numeric(explode('/', $_SERVER['REQUEST_URI'])[3]) && !$this->user->isLoggedIn()) || empty(explode('/', $_SERVER['REQUEST_URI'])[3]))) {
        return $this->redirect('login');
      }
    ...
    */
    public function callback($phpInput) {
      ...
      if($setting['filter-save'] == 0 || $setting['filter-alert'] == 0) {
        $searchCommonReport = $this->database->fetch('SELECT * FROM reports WHERE cookies = :cookies AND dom = :dom AND origin = :origin AND referer = :referer AND uri = :uri AND `user-agent` = :userAgent AND ip = :ip LIMIT 1', [':cookies' => $json->cookies, ':dom' => $json->dom, ':origin' => $json->origin, ':referer' => $json->referrer, ':uri' => $json->uri, ':userAgent' => $json->{'user-agent'}, ':ip' => $userIp]);
      ...
      if(($doubleReport && ($setting['filter-save'] == 1 || $setting['filter-alert'] == 1)) || (!$doubleReport)) {
        if(!empty($json->screenshot)) {
          ...
          $screenshotName = time() . md5($json->uri . time() . bin2hex(openssl_random_pseudo_bytes(16))) . bin2hex(openssl_random_pseudo_bytes(5));
        ...
        $reportId = $this->database->lastInsertId('INSERT INTO reports (`shareid`, `cookies`, `dom`, `origin`, `referer`, `uri`, `user-agent`, `ip`, `time`, `screenshot`, `localstorage`, `sessionstorage`) VALUES (:shareid, :cookies, :dom, :origin, :referer, :uri, :userAgent, :ip, :time, :screenshot, :localstorage, :sessionstorage)',
        ['shareid' => $shareId, ':cookies' => $json->cookies, ':dom' => $json->dom, ':origin' => $json->origin, ':referer' => $json->referrer, ':uri' => $json->uri, ':userAgent' => $json->{'user-agent'}, ':ip' => $userIp, ':time' => time(), ':screenshot' => ((isset($screenshotName)) ? $screenshotName : ''), ':localstorage' => json_encode($json->localstorage), ':sessionstorage' => json_encode($json->sessionstorage)]);
    ...
    * @return string          Javascript payload code
    ...
    public function jsPayload() {
      ...
      return str_replace(
        ['{{domain}}', '{{screenshot}}', '{{customjs}}'],
        [$this->basic->domain(), (($this->database->fetchSetting('screenshot')) ? $this->getFile('screenshot', 'js') : ''), $this->database->fetchSetting('customjs')],
    ...
    * @return string          HTML for page
    ...
    private function parseTemplate($file) {
      $html = str_replace(
        ['{{template}}', '{{menu}}', '{{title}}'],
        [$this->getFile($file), ($this->user->isLoggedIn()) ? $this->basic->htmlBlocks('menu') : $this->basic->htmlBlocks('menuHidden'), ucwords($file)],
